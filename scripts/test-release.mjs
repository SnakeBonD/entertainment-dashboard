...
assert(indexHtml.includes('aria-label="Version 3.2">v3.2'), "La version visible doit être v3.2");
assert(indexHtml.includes("v3.2 · Progression du programme"), "Le pied de page v3.2 est absent");
assert(!indexHtml.includes("v0.8"), "Le HTML contient encore une version v0.8");
assert(readme.includes("version **v3.2**"), "Le README ne présente pas la v3.2");
assert(serviceWorker.includes('`${CACHE_PREFIX}v3.2.0`'), "Le cache applicatif n’est pas en v3.2.0");
...
if (failures.length) {
  console.error("Certification v3.2 échouée :");
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log(`Certification v3.2 réussie : ${ids.length} identifiants uniques, progression du programme quotidien et fonctions historiques conformes.`);
