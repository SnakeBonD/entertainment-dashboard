(() => {
  ...
  function renderTodayProgram() {
    ...
    list.replaceChildren(...items.map((item,index)=>{const row=document.createElement("article");row.className=`today-program-item${item.done?" is-done":""}`;const number=document.createElement("span");number.textContent=item.done?"✓":String(index+1);const details=document.createElement("div");const title=document.createElement("strong");title.textContent=item.title;const meta=document.createElement("small");meta.textContent=item.meta;details.append(title,meta);const complete=document.createElement("button");complete.className="today-program-complete";complete.type="button";complete.textContent=item.done?"Réactiver":"Terminé";complete.setAttribute("aria-pressed",String(item.done));complete.setAttribute("aria-label",`${item.done?"Réactiver":"Marquer comme terminé"} ${item.title}`);complete.addEventListener("click",()=>updateTodayProgram(`${item.title} : progression modifiée.`,()=>window.SnakeBonDTodayProgram.toggleDone(item.id)));const order=document.createElement("div");order.className="today-program-order";[["up","↑","Monter"],["down","↓","Descendre"]].forEach(([direction,symbol,label])=>{const button=document.createElement("button");button.type="button";button.textContent=symbol;button.disabled=direction==="up"?index===0:index===items.length-1;button.setAttribute("aria-label",`${label} ${item.title} dans le programme`);button.addEventListener("click",()=>updateTodayProgram(`${item.title} déplacé dans le programme.`,()=>window.SnakeBonDTodayProgram.move(item.id,direction)));order.append(button);});const link=document.createElement("a");link.className="external-link";link.href=item.url;link.target="_blank";link.rel="noopener noreferrer";link.textContent="Ouvrir ↗";const remove=document.createElement("button");remove.className="today-dismiss";remove.type="button";remove.textContent="Retirer";remove.setAttribute("aria-label",`Retirer ${item.title} du programme du jour`);remove.addEventListener("click",()=>updateTodayProgram(`${item.title} retiré du programme.`,()=>window.SnakeBonDTodayProgram.remove(item.id)));row.append(number,details,complete,order,link,remove);return row;}));
  }

  function updateTodayProgram(message, update) {
    const previousItems = window.SnakeBonDTodayProgram.read();
    update();
    renderTodayProgram();
    offerTodayUndo(message, () => window.SnakeBonDTodayProgram.replace(previousItems));
  }
  ...
  function setupTodayFilters() {
    ...
    clearProgram.addEventListener("click", () => updateTodayProgram("Programme du jour vidé.", () => window.SnakeBonDTodayProgram.clear()));
    document.getElementById("today-program-toggle-all").addEventListener("click", () => { const items=window.SnakeBonDTodayProgram.read();updateTodayProgram(items.every((item)=>item.done)?"Tous les choix ont été réactivés.":"Tous les choix ont été terminés.",()=>window.SnakeBonDTodayProgram.setAllDone(!items.every((item)=>item.done))); });
